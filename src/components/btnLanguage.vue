<template>
<div class="btn-language" :class="{fixed}" @mouseover="show = true" @mouseleave="show = false">
  <transition name="fadeRight">
    <div v-show="show" class="btn-language__content" style="animation-duration: 0.3s">
      <router-link @click="routLinkClick" :to="{ path: language['path'] + this.$route.meta.path }" v-for="language in languages" :key="language['slug']" class="btn-language__link">{{ language['slug'] }}</router-link>
    </div>
  </transition>
