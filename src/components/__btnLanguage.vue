...
<transition name="fadeLeft" style="animation-duration: 0.3s">
  <div v-show="show" class="btn-language__content" v-if="!this.$store.state.projectLanguages && !this.$store.state.partnerLanguages && !this.$store.state.aboutLanguages && !this.$store.state.founderLanguages">
    <router-link
        @click="routLinkClick" 
...
<transition name="fadeLeft" style="animation-duration: 0.3s">
  <div v-show="show" class="btn-language__content" v-if="this.$store.state.projectLanguages">
    <router-link
        v-for="language in languages" 
        @click="routLinkClick" 
        :to="{ path: language['path'] + this.$route.meta.path + this.$store.state.projectLanguages[language['slug']] }" 
        :key="language['slug']" 
        class="btn-language__link">
                {{ language['slug'] }}
    </router-link>
...
<transition name="fadeLeft" style="animation-duration: 0.3s">
  <div v-show="show" class="btn-language__content" v-if="this.$store.state.founderLanguages">
    <router-link
        v-for="language in languages" 
        @click="routLinkClick" 
        :to="{ path: language['path'] + this.$route.meta.path + '/' + this.$store.state.founderLanguages[language['slug']] }" 
        :key="language['slug']" 
        class="btn-language__link">
                {{ language['slug'] }}
    </router-link>
...
<div v-if="mobile" class="btn-language" :class="{fixed: classEl}" @click="show = true" v-click-away="hide">
<transition name="fadeLeft" style="animation-duration: 0.3s">
  <div v-show="show" class="btn-language__content" v-if="!this.$store.state.projectLanguages && !this.$store.state.partnerLanguages && !this.$store.state.aboutLanguages && !this.$store.state.founderLanguages">
    <router-link
        @click="routLinkClick" 
...
export default {
  ...
  methods: {
    ...
    getLanguages(){
        let language_now = this.language;
        let language_def = this.$store.state.language_def;
        let languages_arr = [];
        let fixI = -1;
        for(var i=0; i<this.$route.meta.languages.length; i++){
          if( this.$route.meta.languages[i] === language_def && this.$route.meta.languages[i] !== language_now ){
            fixI++;
            languages_arr[fixI] = {
              slug: this.$route.meta.languages[i],
              path: '',
            };
          }else if( this.$route.meta.languages[i] !== language_now ){
            fixI++;
            languages_arr[fixI] = {
              slug: this.$route.meta.languages[i],
              path: '/' + this.$route.meta.languages[i],
            };
          }
        }
